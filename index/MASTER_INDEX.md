...
Use `facebook-page-management` for authenticated Facebook Page reads/writes, scheduling, media, comments, Page analytics, roles/settings, and Messenger only when fresh live messaging state is verified. Personal Facebook profiles remain outside this skill. Pair with Writing/Editorial, Visual Direction, Video Production, Research, Automations, or Security Engineering only for those distinct layers.

Use `youtube-seo` when the primary problem is YouTube discovery intelligence: query/entity research, Search/Browse/Suggested surface strategy, packaging hypotheses, analytics diagnosis, experiment design/readout, or channel-specific discovery learning. Use `youtube-publishing` for authenticated YouTube reads/transport (including Analytics/Reporting), mutations, and channel operations. When authenticated analytics acquisition and diagnosis are both required, use YouTube Publishing as PRIMARY and YouTube SEO as SUPPORTING.
