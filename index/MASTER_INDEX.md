...
Use `quality-engineering` when the task is primarily to decide what evidence is needed to trust product behavior, design/risk-weight a verification plan, perform independent exploratory/E2E/regression/visual/accessibility checks, characterize defects/flakes, or issue a readiness verdict. Pair with Software Development when fixes or durable test seams are implemented, Interface Design when verifying an agreed UI/UX result, Release Engineering when readiness evidence informs promotion, and Security Engineering when executing agreed security checks. Playwright/browser/mobile/API tools are execution surfaces, not the owning faculty.
