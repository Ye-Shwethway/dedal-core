...
const TOOLS = [
  ...
  tool("youtube_search", "Search YouTube using the connected identity.", { profile_alias: str(), q: str(), type: { type: "string", enum: ["video","channel","playlist"] }, order: { type: "string", enum: ["date","rating","relevance","title","videoCount","viewCount"] }, max_results: integer(1,50) }, true, ["profile_alias","q"]),
...
async function callGateway(name, a, env) {
  ...
  const routes = {
    ...
    youtube_search: dataApi("search", "list", { part: "snippet", q: a.q, type: a.type, order: a.order, maxResults: a.max_results || 25, forMine: a.type === "video" ? true : undefined }),
